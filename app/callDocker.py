...
def call(container,command,cwd='',paths={},remove=True,cpu_set='',sig_default=True):
    ...
    output = b''
    # try block to run container
    try:
        ### format cpu set for control of cpus
        if cpu_set:
            cpu_set = int(str(int(cpu_set))+"00000")
            container_obj = client.containers.run(container,command,user=user,volumes=volumes,working_dir=cwd,remove=remove,detach=True,cpu_period=100000,cpu_quota=cpu_set,labels={"prog":"mjolnirTree"})
        else:
            container_obj = client.containers.run(container,command,user=user,volumes=volumes,working_dir=cwd,remove=remove,detach=True,labels={"prog":"mjolnirTree"})
    except:
